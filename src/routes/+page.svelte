<script lang="ts">
    ...
    onMount(() => { 
        ...
        if(urlParams.has("lang")) { userLang = urlParams.get("lang") || userLang }

        if(userLang !== "es"){ localization = getLocalization(userLang) }
...
<div>
    ...
    <Job title={ localization.titles.job } animationDelay = { 1000 } experiences = { localization.personal.job ||  [] } filter = { filter } />
    <Skill titles={ localization.titles.skill } animationDelay = { 1500 } skills = { localization.personal.skill || [] } filter = { filter } />
    <Education title={ localization.titles.education } animationDelay = { 2000 } experiences = { localization.personal.education } />
    <Others titles={ localization.titles.other } animationDelay = { 5000 } personal = { localization.personal } filter = { filter } />
</div>
<footer transition:fade={{delay: 6000, duration: 1000}}>
    <div style="display: flex; justify-content: flex-end; gap: 1em;">
        ...
        {/each}
        <a href="{url}">{ localization.titles.web || "Versión Web" }</a>
    </div>
</footer>
